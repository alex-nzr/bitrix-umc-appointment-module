...
class OneCDataState {
    ...
    private ajaxUrl = '/bitrix/services/main/ajax.php';
    private controller = 'anz:appointment.Appointment';
    ...
    public async loadData(): Promise<IResponse>{
        ...
        try{
            ...
            if (data && data.errors){
                return this.makeResponse(false, data.errors);
            }else{
    ...
    private async getAppointmentData(): Promise<IOneCdata> {
        try {
            ...
            const action = `${this.controller}.getData`;
            // @ts-ignore
            const sessid = window.BX.bitrix_sessid();
            const response = await fetch(`${this.ajaxUrl}?sessid=${sessid}&action=${action}`, this.requestParams);
            if (response.ok) {
    ...
    public makeResponse(success: boolean, error: string|[] = ''): IResponse
    {
        ...
        if (error){
            // @ts-ignore
            response.error = Array.isArray(error) ? error[0]['message'] : error;
        }
        return response;
    }
