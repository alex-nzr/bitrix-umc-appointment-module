...
window.appointmentWidget = {
	...
	start: function(){
		...
		BX.ajax.runAction('firstbit:appointment.oneCController.getClinics', {
			data:{
			...
			.then((response) =>
			{
				this.processResponse(response, this.dataKeys.clinicsKey);
				BX.ajax.runAction('firstbit:appointment.oneCController.getEmployees', {
					data:{
						sessid: BX.bitrix_sessid()
					}
				})
					.then((response) =>
					{
						this.processResponse(response, this.dataKeys.employeesKey);
						BX.ajax.runAction('firstbit:appointment.oneCController.getSchedule', {
							data:{
								sessid: BX.bitrix_sessid()
							}
						})
							.then((response) =>
							{
								this.processResponse(response, this.dataKeys.scheduleKey);
								this.loaded = true;
			...
			.catch(e => console.log(`Loading data error - ${JSON.stringify(e)}`))
	},

	processResponse: function (response, dataKey) {
		if (!response){
			...
			if (response.data?.error)
			{
				...
				switch (dataKey) {
					...
					case 'sendOrder':
						...
						break
					default:
						throw new Error(`${dataKey} is unexpected key`)
				}
			}
		}
		else{
	...
	afterLoadActions: function (){
		if (this.loaded){
			...
			console.log("After loading data error")
		}
	},
	...
	changeStep: function(dataKey, target){
		switch (dataKey) {
			case this.dataKeys.clinicsKey:
				...
				if (this.useServices)
				{
					this.selectionNodes[this.dataKeys.servicesKey].blockNode.classList.add('loading');
					BX.ajax.runAction('firstbit:appointment.oneCController.getNomenclature', {
						data: {
							clinicGuid: target.dataset.uid
						}
					}).then((response) => {
						this.processResponse(response, this.dataKeys.servicesKey);
						this.bindServicesToSpecialties();
						this.selectionNodes[this.dataKeys.servicesKey].blockNode.classList.remove('loading');
					}).catch(e => console.log(`Services loading error - ${JSON.stringify(e)}`))
				}
				break;
	...
	submit: async function(event){
		...
		if (this.checkRequiredFields())
		{
			...
			{
				if (this.selectionNodes.hasOwnProperty(key) && this.filledInputs[key])
				{
	...
	sendOrder: function (params) {
		...
		.then((response) => {
				this.processResponse(response, 'sendOrder');
		})
		.catch(e => {
			console.log(`Send order error ${JSON.stringify(e)}`);
	...
	bindServicesToSpecialties: function() {
		const services  = this.data[this.dataKeys.servicesKey];
		const employees = this.data[this.dataKeys.employeesKey];
	...
	createIdFromName: function(specialty) {
		return btoa(unescape(encodeURIComponent(specialty)));
	}
}
