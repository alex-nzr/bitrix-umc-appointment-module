...
class OneCDataState
{
    private readonly pathToApi: string = '/widgetV2/api/ajax.php';
    ...
    private async getAppointmentData(): Promise<IOneCdata> {
        try {
            const response = await fetch(`${this.pathToApi}?action=getAppointmentData`, this.requestParams);
            if (response.ok) {
    ...
    public async sendOrder (params: IOrderParams): Promise<IResponse> {
        ...
        try {
            ...
            const response = await fetch(`${this.pathToApi}?action=createOrder`, this.requestParams);
    ...
    private async sendToEmail (params: IOrderParams) {
        this.requestParams.body = JSON.stringify(params);
        await fetch(`${this.pathToApi}?action=sendEmail`, this.requestParams);
    }
    ...
    getClinicUidByEmployeeUid(employeeUid: string) {
        const employee = this.data.employees[employeeUid];
        return employee ? String(employee.clinicUid) : '';
    }

    getEmployeeFirstSpecialtyUid(employeeUid: string) {
        const employee = this.data.employees[employeeUid];
        return employee?.specialties ? String(Object.keys(employee.specialties)?.[0]) : '';
    }

    getFirstEmployeeUidByServiceUid(searchServiceUid: string) {
        let foundedEmployeeUid = '';
        for (const employeeUid in this.data.employees)
        {
            if (foundedEmployeeUid.length > 0)
            {
                break;
            }

            for (const serviceUid in this.data.employees[employeeUid].services)
            {
                if (serviceUid === searchServiceUid)
                {
                    foundedEmployeeUid = employeeUid;
                    break;
                }
            }
        }
        return foundedEmployeeUid;
    }

    getServiceDurationByUid(serviceUid: string) {
        return Number(this.data.nomenclature[serviceUid]?.duration);
    }

    getDefaultClinicUid() {
        let clinic = this.data.clinics.find((item: IOneCClinic) => item.isDefault);

        //Если клиника по умолчанию не найдена, возвращаем первую из списка
        return clinic ? clinic.uid : String(this.data.clinics[0]?.uid);
    }
}

const oneCDataState = new OneCDataState();
export default oneCDataState;